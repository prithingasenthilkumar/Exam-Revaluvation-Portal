...
const StudentDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      setLoading(true);
      const response = await LocalStorageService.getStudentRequests(user.id);
      if (response.success) {
        ...
        setDashboardData({ stats, recentRequests: requests.slice(0, 5) });
      } else {
                ...
                <tbody className="divide-y divide-[#dbe0e6] dark:divide-gray-700">
                  {requests.map((req) => (
                    <tr key={req.id} className="hover:bg-[#f8f9fa] dark:hover:bg-gray-700/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-[#617589] dark:text-gray-400">{new Date(req.createdAt).toLocaleDateString()}</td>
                      <td className="px-6 py-4">
                        <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider text-green-700 bg-green-100 dark:bg-green-900/30`}>
                          Paid
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center gap-1.5 py-1 px-2.5 rounded-full text-[10px] font-bold ${getStatusStyles(req.status.toUpperCase())}`}>
...
const NotificationBanner = () => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      if (user) {
        const response = await LocalStorageService.getNotifications(user.id);
        if (response.success) {
    ...
    <div className="mt-2 space-y-2">
      {notifications.map(notification => (
        <div key={notification.id} className={`p-3 rounded-lg text-sm ${
          notification.type === 'approved' ? 'bg-green-100 text-green-800 border border-green-200' :
          notification.type === 'rejected' ? 'bg-red-100 text-red-800 border border-red-200' :
